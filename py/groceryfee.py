...
# for ① "num1[x|*|X]num2", e.g. 88x3 or 88*3 or 88X3
            if ptnNumXNum.match(arg):
                multiplicandLst = list(arg.split(
                    multiplySymb) for multiplySymb in multiplySymbols if arg.find(multiplySymb) > -1)
                #multiplicandLst = [arg.split(multiplySymb) for multiplySymb in multiplySymbols if arg.find(multiplySymb) > -1]
                print(
                    'multiplicandLst before flattening:{}'.format(multiplicandLst))
                multiplicandLstFlat = [
                    item for splitLst in multiplicandLst for item in splitLst]
                print('multiplicandLst after flattening:{}'.format(
                    multiplicandLstFlat))
...
# todo difficult
# for ② args consists solely of symbols in ['x', '*', 'X'], multiply
# previous and following numbers, e.g. 88 x 3
...
# tax 12% floor
    ...
    feeDict = {
        'Before Tax': {
            'from func "sum"': varSum, 'from func "reduce"': varSumByReduce}, '8%': {
            'from func "sum"': varSum8, 'from func "reduce"': varSumByReduce8}, '8% floor': {
                'from func "sum"': varSum8flr, 'from func "reduce"': varSumByReduce8flr}, '10%': {
                    'from func "sum"': varSum10, 'from func "reduce"': varSumByReduce10}, '10% floor': {
                        'from func "sum"': varSum10flr, 'from func "reduce"': varSumByReduce10flr}, '12%': {
                            'from func "sum"': varSum12, 'from func "reduce"': varSumByReduce12}, '12% floor': {
                                'from func "sum"': varSum12flr, 'from func "reduce"': varSumByReduce12flr}}
